...
import { GameCell, GameCellAndCoords, GameGrid, GameState, PlacementSummary, PlayedWord, TurnRule } from "./game";
import { Bonus, Letter, bonusAbbreviation } from "./tile_data";
...
function wordId(word: PlayedWord): string {
    ...
    const wordSrt = word.word.map((l, idx)=> {
...
function playedWordFromWalk(
    ...
    for (const { cell: {data: {letter, bonus}}, coords } of walk) {
        word.push(letter!);
        bonuses.push(bonus);
    }
...
function detectWords(
    ...
    for (const cellAndCoords of walk) {
        const { cell: {data: { letter}} } = cellAndCoords;
        if (letter) {
            if (!currentWord) {
    ...
    console.log({
        start: `${walk[0].coords.x}, ${walk[0].coords.y}`,
        direction,
        word: words.map(w => wordId(w)).join('| '),
    })
...
function summarizePlacement(
    ...
    console.log("SCANNING LAST GRID")
    const currentWords = scanForWords(grid);
    console.log("SCANNING NEXT GRID")
    ...
    return Result.success({ wordsPlayed: wordsPlayed, lettersPlayed: lettersPlayed });
}

function scoreValidPlacement(
...
export function validateTurn(
    ...
    if (Result.isFailure(placementResult)) {
        return { kind: 'invalid', message: placementResult.message || 'Invalid turn' };
    } 
